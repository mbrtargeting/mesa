...
module.exports = {
  ...
  'advisory locks and transactions (long version)'(test) {
    ...
    const addressPromises = [700, 600, 500, 400, 300, 200, 100].map(delay => mesa.wrapInTransaction(function(transaction) {
      const withTransaction = bitcoinReceiveAddress.setConnection(transaction);
      return withTransaction
        ...
        .then(function(address) {
          ...
          return Promise.delay(delay).then(() => withTransaction
            .where({id: address.id})
            .returnFirst()
            .delete()
            .then(() => address));
    ...
    return Promise.all(addressPromises).then(function(addresses) {
      ...
      addresses.forEach(function(address) {
        if (address != null) {
          ...
          existing.forEach(existing => test.notEqual(address.id, existing.id));
