...
Mesa.prototype = {
  ...
  runQueue(queue, value, origin = value) {
    const context = this;
    const reducer = (soFar, step) => soFar.then(_.bind(step, context, _, origin));
  ...
  afterQuery(results, options) {
    ...
    return this.runQueue(options.after, rows, origin)
      .map((row) => this.runQueue(options.afterEach, row, origin))
      .then(function(rows) {
        if (typeof debug === 'function') {
  ...
  insert(...args) {
    ...
    const origin = returnFirst ? _.cloneDeep(records[0]) : _.cloneDeep(records);
    ...
    return this.runQueue(this._queueBeforeInsert, records)
      .map((record) => this.runQueue(this._queueBeforeEachInsert, record))
  ...
  update(update) {
    const that = this;
    const debug = this._debug;
    const origin = _.cloneDeep(update);
