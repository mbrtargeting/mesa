...
Mesa.prototype = {
  ...
  runQueue: function(queue, value, rest) {
    var context, reducer;
    context = this;
    reducer = function(soFar, step) {
      return soFar.then(_.bind(step, context, _, rest));
    };
  ...
  afterQuery: function(results, options) {
    ...
    return this.runQueue(options.after, rows, options.rest).map((function(_this) {
      return function(row) {
        return _this.runQueue(options.afterEach, row, options.rest);
      };
  ...
  insert: function() {
    var args, debug, records, records_, returnFirst, that;
    args = 1 <= arguments.length ? slice.call(arguments, 0) : [];
    that = this;
    debug = this._debug;
    ...
    return this.runQueue(this._queueBeforeInsert, records, records).map((function(_this) {
      return function(record) {
        return _this.runQueue(_this._queueBeforeEachInsert, record, record);
      };
      ...
      return that.query(that._mohair.insert(records)).then(function(results) {
        return that.afterQuery(results, {
          rest: returnFirst ? records_[0] : records_,
  ...
  update: function(update) {
    ...
    return this.runQueue(this._queueBeforeEachUpdate, update, update).then(function(update) {
      if (typeof debug === "function") {
