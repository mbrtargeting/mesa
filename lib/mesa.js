...
helpers.schemaAwareEscape = string => string.split('.').map(str => `\"${str}\"`).join('.');
...
helpers.ignoredArgumentWarning = receiver => `you called ${receiver} with an argument but ${receiver} ignores all arguments. ${receiver} returns a promise and maybe you wanted to call that promise instead: ${receiver}.then(function(result) { ... })`;
...
helpers.normalizeLink = function(leftTable, rightTable, immutableLink) {
  ...
  if (link.forward == null) { link.forward = true; }
  if (link.first == null) { link.first = false; }
...
Mesa.prototype = {
  ...
  when(condition, fn, ...args) {
    if (condition) { return this.call(fn, ...Array.from(args)); } else { return this; }
  },
...
// mass assignment protection
  ...
  _isUnsafe: false,
  unsafe(isUnsafe) {
    if (isUnsafe == null) { isUnsafe = true; }
...
// pass throughs to the underlying mohair query builder instance
  ...
  where(...args) {
    return this.fluent('_mohair', this._mohair.where(...Array.from(args || [])));
  },
  having(...args) {
    return this.fluent('_mohair', this._mohair.having(...Array.from(args || [])));
  },
  join(...args) {
    return this.fluent('_mohair', this._mohair.join(...Array.from(args || [])));
  },
  select(...args) {
    return this.fluent('_mohair', this._mohair.select(...Array.from(args || [])));
  },
  limit(arg) {
  ...
  returning(...args) {
    return this.fluent('_mohair', this._mohair.returning(...Array.from(args || [])));
  },
  distinct(...args) {
    return this.fluent('_mohair', this._mohair.distinct(...Array.from(args || [])));
  },
  for(...args) {
    return this.fluent('_mohair', this._mohair.for(...Array.from(args || [])));
  },
  window(...args) {
    return this.fluent('_mohair', this._mohair.window(...Array.from(args || [])));
  },

  combine(...args) {
    return this.fluent('_mohair', this._mohair.combine(...Array.from(args || [])));
  },
  union(...args) {
    return this.fluent('_mohair', this._mohair.union(...Array.from(args || [])));
  },
  unionAll(...args) {
    return this.fluent('_mohair', this._mohair.unionAll(...Array.from(args || [])));
  },
  intersect(...args) {
    return this.fluent('_mohair', this._mohair.intersect(...Array.from(args || [])));
  },
  intersectAll(...args) {
    return this.fluent('_mohair', this._mohair.intersectAll(...Array.from(args || [])));
  },
  except(...args) {
    return this.fluent('_mohair', this._mohair.except(...Array.from(args || [])));
  },
  exceptAll(...args) {
    return this.fluent('_mohair', this._mohair.exceptAll(...Array.from(args || [])));
  },
...
// promise wrapper around node-postgres
  ...
  wrapInConnection(block) {
    return this.getConnection().then(({connection, done}) => block(connection).finally(() => typeof done === 'function' ? done() : undefined));
  },
  ...
  query(sqlOrFragment, params) {
    ...
    const that = this;
    const debug = this._debug;
    return this.wrapInConnection(connection => new Promise(function(resolve, reject) {
      if (typeof debug === 'function') {
        debug('query', 'before', {
          sql,
          params
  ...
  wrapInTransaction(block) {
    ...
    return this.wrapInConnection(function(connection) {
      ...
      return withConnection.query('BEGIN;')
      .then(() => block(connection)).then(function(result) {
        if (typeof debug === 'function') {
          debug('transaction', 'commit', {}, {connection, block}, that);
        }
        return withConnection.query('COMMIT;').then(() => result);}).catch(function(error) {
        if (typeof debug === 'function') {
          debug('transaction', 'rollback', {error}, {
            connection,
            block
          }, that);
        }
        return withConnection.query('ROLLBACK;').then(() => Promise.reject(error));
      });
    });
  },
...
// command: these functions have side effects
  ...
  insert(...args) {
    ...
    .then(function(records) {
      ...
      return that.query(that._mohair.insert(records)).then(results => that.afterQuery(results, {
            returnFirst,
  ...
  update(update) {
    ...
    return this.runQueue(this._queueBeforeEachUpdate, update).then(function(update) {
      ...
      return that.query(that._mohair.update(update)).then(results => that.afterQuery(results, {
  ...
  delete() {
    ...
    return that.query(that._mohair.delete()).then(results => that.afterQuery(results, {
          returnFirst: that._returnFirst,
...
// query: these functions have no side effects
  ...
  find(arg) {
    ...
    return that.query(this).then(results => that.afterQuery(results, {
          returnFirst: that._returnFirst,
  ...
  exists(arg) {
    ...
    return this.query(this._mohair.limit(1)).then(results => (results.rows != null) && (results.rows.length !== 0));
  },
...
// embed
  ...
  baseEmbed(originalRecords, includes) {
    ...
    const reducer = (soFar, include) => // run in series
          ...
          .then(function(nextRecords) {
            ...
            groupedByFirst =
                (groupedByFirst == null) ?
                    groupedByCurrent
                    :
                    _.mapValues(groupedByFirst, records => // forward bucket to the next layer
                        // by replacing records by the records they are
                        // associated with
                        _.reduce(records, function(acc, record) {
                          records = groupedByCurrent[record[include.left]];
                          if (records != null) { return acc.concat(records); } else { return acc; }
                        }, []));
    ...
    return includes.reduce(reducer, Promise.resolve()).then(() => // finally return the original records
        originalRecords);
  },
  ...
  embed(records, ...args) {
    ...
    return this.baseEmbed(records, helpers.normalizeIncludeArguments(this, ...Array.from(args)));
  },
...
const payload = function(f, ...args) {
  if (args.length === 0) { return f; } else { return _.partialRight(f, ...Array.from(args)); }
};
...
const setQueueProperties = function(object, suffix) {
  ...
  return object[setterPropertyName] = function(...args) {
    return this.fluent(dataPropertyName, this[dataPropertyName].concat([payload(...Array.from(args || []))]));
  };
};
...
for (let phase of ['Select', 'Insert', 'Update', 'Delete']) {
...
Mesa.prototype.queueBeforeEach = function(...args) {
  ...
  ['Insert', 'Update'].forEach(function(phase) {
    ...
    return object[propertyName] = object[propertyName].concat([payload(...Array.from(args || []))]);});
  return object;
};
...
Mesa.prototype.isInstance = (helpers.isInstance = object => object instanceof Mesa);
